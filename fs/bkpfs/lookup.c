...
struct dentry* bkpfs_get_bkp_dentry(struct dentry* lower_dir_dentry , const char* name, int is_neg_dentry)
{
	int err = 0;
	struct dentry *ret_dentry;
	struct qstr this;
	...
	ret_dentry = d_lookup(lower_dir_dentry, &this);
	/*
	...
	if (ret_dentry == NULL && !is_neg_dentry){
		err = -ENOENT;
		goto out;
	}

	if(is_neg_dentry){
		ret_dentry = d_alloc(lower_dir_dentry, &this);
		if (!ret_dentry) {
			err = -ENOMEM;
			goto out;
		}
				      ...
				      struct path *lower_parent_path)
{
	...
	if (!err) {
		...
		if((vfs_getxattr(lower_path.dentry, BKPFS_XATTR_NAME, NULL, 0) > 0) || S_ISDIR(lower_path.dentry->d_inode->i_mode)){
			/* For non backup files create upper layer dentry link and inode */
			...
			/* Special Case:
			 * This is backup dentry don't create bkpfs link or upper inode for this
			 * We set err to ENOENT and don't create negative dentry as well.
			 */
			printk(KERN_INFO "INFO:: backup file:%s lookup successfull\n", name);
			err = -ENOENT;
		}
		goto out;
	}
			     ...
			     unsigned int flags)
{
	...
	printk(KERN_INFO "Lookup::dname->%s",dentry->d_name.name);
