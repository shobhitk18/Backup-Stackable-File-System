...
static int bkpfs_show_options(struct seq_file *m, struct dentry *root)
{
	int rc = 0;
	struct super_block *sb = root->d_sb;
	struct mnt_opt_info *mnt_opts = &BKPFS_SB(sb)->mnt_opts;
	UDBG;

	if (mnt_opts->maxvers){
		seq_printf(m, ",max_versions=%d", mnt_opts->maxvers);
		printk(KERN_INFO "max versions=%d\n", mnt_opts->maxvers);
	}
	if (mnt_opts->bkp_threshold){
		seq_printf(m, ",backup threshold(Bytes)=%d", mnt_opts->bkp_threshold);
		printk(KERN_INFO "backup_threshold=%d\n", mnt_opts->bkp_threshold);
	}
...
const struct super_operations bkpfs_sops = {
	...
	.show_options   = bkpfs_show_options,
};
